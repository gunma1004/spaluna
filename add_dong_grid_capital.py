...
# 시 단위 메인 폴더 (수원, 성남, 고양, 용인, 안양, 안산)에서 구/동 링크 연결
CITY_MAP = {
    "suwon": ("수원시", [
...
# 1. 경기/인천 각 구별 index.html 및 세부 동 html 에 동 바로가기 그리드 삽입
for folder, (kr_name, dongs) in DETAILED_REGIONS.items():
    if not os.path.exists(folder):
        continue
    
    grid_items = "".join([f'<a href="{f}.html" class="gu-item">{name}</a>\n            ' for f, name in dongs])
    gu_grid_html = f"""
        <h2 class="section-title">{kr_name} 세부 동 바로가기</h2>
        <div class="gu-grid">
            {grid_items}
        </div>
    """
    ...
    for file in os.listdir(folder):
        ...
        # gu-grid 가 이미 있으면 교체, 없으면 container 닫기 직전에 추가
        if '<div class="gu-grid">' in content:
            content = re.sub(r'<h2 class="section-title">.*?세부.*?</h2>\s*<div class="gu-grid">.*?</div>', gu_grid_html.strip(), content, flags=re.DOTALL)
        else:
            content = re.sub(r'(</div>\s*<footer>)', rf'{gu_grid_html}\n    \1', content, flags=re.DOTALL)

        with open(file_path, "w", encoding="utf-8") as f:
            f.write(content)
        count += 1

# 2. 수원/성남/고양 등 시 단위 대표 폴더 index.html 에 하위 구 바로가기 그리드 삽입
for city_folder, (city_kr, gus) in CITY_MAP.items():
    if not os.path.exists(city_folder):
        continue
    grid_items = "".join([f'<a href="{link}" class="gu-item">{name}</a>\n            ' for link, name in gus])
    city_grid_html = f"""
        <h2 class="section-title">{city_kr} 전체 구 바로가기</h2>
        <div class="gu-grid">
            {grid_items}
        </div>
    """
    ...
    if os.path.exists(file_path):
        with open(file_path, "r", encoding="utf-8") as f:
            content = f.read()
        if '<div class="gu-grid">' in content:
            content = re.sub(r'<h2 class="section-title">.*?전체 구.*?</h2>\s*<div class="gu-grid">.*?</div>', city_grid_html.strip(), content, flags=re.DOTALL)
        else:
            content = re.sub(r'(</div>\s*<footer>)', rf'{city_grid_html}\n    \1', content, flags=re.DOTALL)
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(content)
        count += 1

print(f"🎉 총 {count}개 경기/인천 전체 페이지에 구/동 바로가기 버튼 UI가 완벽하게 연결되었습니다.")
